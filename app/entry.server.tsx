...
async function handleNodeRequest(
  ...
  return new Promise((resolve, reject) => {
    let didError = false;
    const { pipe, abort } = (ReactDOM as any).renderToPipeableStream(jsx, {
      [isBot ? 'onAllReady' : 'onShellReady']: async () => {
        const { PassThrough } = await safeRequireNodeDependency('node:stream');
        const { createReadableStreamFromReadable } = await safeRequireNodeDependency('@remix-run/node');
