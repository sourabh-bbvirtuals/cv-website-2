...
export async function loader({ params, request }: DataFunctionArgs) {
  ...
  try {
    ...
    const optionGroups = allOptionGroups.filter((og) => usedGroupIds.has(og.id));
          ...
          faculties = faculties.map((fd: any) => {
            if (!fd.image) {
              const match = facultyInfos.find(
                (f: any) => f.name && f.name.toLowerCase() === fd.name.toLowerCase(),
              );
          ...
          ...(hasOptions && {
            ...
            variants: variants.map((v) => ({
              ...
              options: (v.options || []).map((o: any) => ({
                id: o.id,
                name: o.name,
                group: o.group ? { id: o.group.id, name: o.group.name } : undefined,
              })),
            })),
          }),
        }
