  ...
  try {
    ...
    while (true) {
      ...
      const result = (await response.json()) as {
        data?: { products?: { totalItems: number; items: Array<{ slug: string }> } };
      };
...
function buildSitemapXml(urls: Array<{ loc: string; priority?: string; changefreq?: string }>) {
  const entries = urls
    .map(
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  urls.push({ loc: `${SITE_URL}/our-courses`, priority: '1.0', changefreq: 'daily' });

  for (const slug of productSlugs) {
    urls.push({ loc: `${SITE_URL}/our-courses/${slug}`, priority: '0.8', changefreq: 'weekly' });
  }

  urls.push({ loc: `${SITE_URL}/blogs`, priority: '0.8', changefreq: 'daily' });

  for (const slug of blogSlugs) {
    urls.push({ loc: `${SITE_URL}/blogs/${slug}`, priority: '0.6', changefreq: 'weekly' });
  }

  urls.push({ loc: `${SITE_URL}/free-resources`, priority: '0.9', changefreq: 'daily' });

  for (const tab of FREE_RESOURCE_TABS) {
    urls.push({ loc: `${SITE_URL}/free-resources/${tab}`, priority: '0.7', changefreq: 'weekly' });
  }
