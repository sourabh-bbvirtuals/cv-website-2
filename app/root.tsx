...
export default function App() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && typeof window.gtag === 'function') {
      window.gtag('config', 'G-001NR9NF1Y', { page_path: location.pathname + location.search });
    }
  }, [location]);
  ...
  return (
    <html lang={locale} dir={i18n.dir()} id="app">
      <head>
        ...
        <title>{title}</title>
        <script async src="https://www.googletagmanager.com/gtag/js?id=G-001NR9NF1Y" />
        <script dangerouslySetInnerHTML={{ __html: `window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('config','G-001NR9NF1Y');` }} />
      </head>
      <body
